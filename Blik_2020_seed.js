...
import file,{details,read,output} from "./Blik_2020_file.js";
...
export async function open(route,protocol="http")
{if(!route)
 throw Error("no route module specified");
 let info=await import("./package.json").then(info=>info.default);
 let keys=await [info,"parameters",protocol].reduce(async(module,key,index,trace)=>
...
{await npm(Object.entries(info.dependencies).map(([module,version])=>module+"@"+version));
 ...
 if(tests)await import("./Blik_2020_integrity.js").then(({check})=>check(route,tests));
...
[...protocol.globalAgent.protocol=="https:"?[await certify(keys.certification,keys.distinguishedname)]:[]
...
{let source=(request.connection?request.connection.remoteAddress:"")+request.url;
 ...
 if(format)request.body=format.parse(request.body);
 ...
 let data=this[request.url[0]]
?await [this,...request.url,undefined].reduce(route=>
{let step=request.url.shift();
 !route[step]&&step&&request.url.unshift(step);
 step=route[step]||route[request.method]||route;
 return typeof step=="function"?step.bind(route)(request):step;
})
:await (file[request.method]||file.get)(request,"binary").catch(body=>{status:404,body});
 // MATCH RESIDUAL PATH WITHIN DATA
 data=[data,...request.url].reduce((data,step)=>
 data?data[step]||data:Error("no such file or function"));
 // PREPARE RESPONSE
 data=data.status
?{status:200,type:"json",...data}
:data instanceof Error
?{status:500,body:data.toString(),type:"txt"}
:data.outerHTML
...
,tool,"dist",!index||!tools[++index]?"es":""
,(index?"index":tool)+".js"
].filter(Boolean).join("/"));
...
//},{});
 ...
 spawn("npm",note([command,"-g",...modules])).on("error",note).on("exit",exit=>
 setTimeout(time=>resolve(command),300)).stdout.on("data",data=>
 command+="\n"+data.toString("utf8"))).then(note);
