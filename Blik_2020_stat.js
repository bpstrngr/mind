import document,{window} from "./Blik_2020_document.js";
...
{let graph=document.createElement("div");
 ...
 let line=graph.appendChild(document.createElement("div"));
...
{year=entry.substring(0,4);
 ...
 var yearmark=graphic.appendChild(document.createElement("span"));
 yearmark.appendChild(document.createTextNode(year+"\n"));
 ...
 let timestamp=graphic.appendChild(document.createElement("span"));
 timestamp.appendChild(document.createTextNode(entry.substring(4)));
...
{if(this.naturalHeight-this.naturalWidth>10){this.style.width="auto";let fit=((this.naturalHeight-this.naturalWidth)/4.5)+"px";console.log(fit);this.style.paddingLeft=fit,this.style.paddingRight=fit}
 ...
 graphic.appendChild(document.createTextNode(history[entry].description));
 graphic.appendChild(document.createTextNode("\n"));
...
{//var model=await fetch(source).then(resource=>resource.json());
 ...
 catch(fail){return document.createTextNode(fail.toString());}
...
?value[0]=="<"
?window.document.createRange().createContextualFragment(value).firstChild
...
:document.scan(value);
 ...
 piece.appendChild(window.document.createElementNS("http://www.w3.org/2000/svg","title")).textContent=key;
 //piece.style.width=piece.style.width||window.innerWidth/(items.length+1)*(items.length<window.innerWidth/50?1:(items.length/50));
 //if(value.length)piece.querySelector("img").onload=(event)=>{let canvas=document.createElement("canvas");canvas.width=event.target.naturalWidth,canvas.height=event.target.naturalHeight;canvas.getContext("2d").drawImage(event.target,0,0);}})
...
{let shade=depth>2&&Array.isArray(palette)?d3.scaleLinear().range([palette[index],(palette[index]||palette[0]||spectrum(index/records.length).replace(")","0)")).replace("0)","1)")]):palette;
 ...
 if(depth==2)
 top.lastChild.appendChild(document.createElement("td")).appendChild(document.createElement("span")).textContent=Object.keys(source)[0]=="CICES"?"adaptáció":Object.keys(source)[0]=="functional ecology"?"ecological economics":"ökológiai közgazdaságtan";
...
{[table.appendChild(document.createElement("tr")),index+1,...record,index+1
 ...
 ["",...matrix].reduce((row,field,index)=>document.scan({"td":{"#text":index||field}},row),table.appendChild(document.createElement("tr")));
 ...
 let input=table.appendChild(document.createElement("tr"));input.setAttribute("id","input");
 let output=table.appendChild(document.createElement("tr"));output.setAttribute("id","output");
 ...
 let ratio=table.appendChild(document.createElement("tr"));ratio.setAttribute("id","ratio");
 ...
 input=table.appendChild(document.createElement("tr"));input.setAttribute("id","exposure");
 output=table.appendChild(document.createElement("tr"));output.setAttribute("id","imposure");
 ...
 let balance=table.appendChild(document.createElement("tr"));balance.setAttribute("id","balance");
 ...
 let model=table.appendChild(document.createElement("tr"));model.setAttribute("id","model");
