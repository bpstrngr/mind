...
{if(typeof source=="object")
 ...
 return await import("./"+source).then(module=>module.default).then(dig.bind(0,source));
 let response=await fetch(source||get,{method:"get",mode:"no-cors"});
 if(response.status!=200&&!source.startsWith("http"))
 if(response.status==404)
 response=await fetch("get?name="+source).then(response=>response.json()).then(list=>
 note(list)&&list.length?fetch(list[0].name):"missing source")
...
?response.json().then(json=>json.type=="Buffer"?JSON.parse(new TextDecoder("utf-8").decode(new Uint8Array(json.data))):typeof json=="string"?JSON.parse(json):json).then(dig.bind(0,source))
...
{let stack=Error().stack.split(/\n */)
 ...
 console.log(!globalThis.window?"\x1b[30m"+place+"\x1b[0m":place);
 return note[0]
};
